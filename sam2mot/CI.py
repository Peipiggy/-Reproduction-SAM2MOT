...
class CI:
    ...
    def get_trackinfo(self, frame_idx, inference_state,  device, upscale=True): #TODO
        ...
        for obj_id in inference_state["obj_ids"]:
            ...
            if upscale: # 上采样到原始视频尺寸
                mask = F.interpolate(mask, size=(H_orig, W_orig), mode="bilinear", align_corners=False)[0, 0]
            ...
            if ysx.shape[0] > 0:
                ...
                bbox = torch.tensor([[x1, y1, x2, y2]], dtype=torch.float32, device=mask.device)
            else:
                bbox = torch.tensor([[0, 0, 0, 0]], dtype=torch.float32, device=mask.device)
            ...
            history_logits = [all_outputs[frame_id]["object_score_logits"].item() for frame_id in recent_frames]
    ...
    def remove_occlusion(self, inference_state, frame_id, device):
        ...
        '''
        tck_ids, tck_logits, tck_masks, tck_logits_history, tck_bboxes = self.get_trackinfo(frame_idx=frame_id, inference_state=inference_state, device=device)
        removed = []
        for i in range(len(tck_ids)):
            for j in range(i+1, len(tck_ids)):
                ...
                logit_history_i, logit_history_j = tck_logits_history[i], tck_logits_history[j]
                miou = self.miou(mask1=mask_i, mask2=mask_j) # 为什么要用miou呢？？？？SAM2生成的miou几乎不可能重叠呀
                ...
                if miou>self.overlap_th: # 发生occlusion
                    ...
                    if abs(logit_i-logit_j)>self.similar_th: # 相差较大
                        ...
                        removed.append(id_i if np.var(logit_history_i)>np.var(logit_history_j) else id_j)
