...
class TMS:
    ...
    def TrajectroyManage(self, frame_id, inference_state, det_bboxes, det_logits, device='device'):
        '''
        传入detector和inference_state进行轨迹管理
        '''
        # 获取tracking的信息
        tck_ids, tck_logits, tck_bboxes, tracked_mask, _ = self.get_trackinfo(frame_idx=frame_id, inference_state=inference_state, device=device)
        # 根据detector返回的信息更新TMS的Tracklet
        self.update(tck_ids=tck_ids, tck_logits=tck_logits, tck_bboxes=tck_bboxes, frame_id=frame_id)
        # 将det和tck匹配起来，保留大于0.5confidence的det_bboxes
        matched_pairs, unmatched_dets, det_bboxes, det_logits = self.get_bboxes(det_bboxes=det_bboxes, det_logits=det_logits)
        # object addition
        added = self.object_addition(unmatched_dets=unmatched_dets, det_bboxes=det_bboxes, det_logtis=det_logits, frame_id=frame_id, tracked_mask=tracked_mask, device=device)
        # object removal
        removed = self.object_removal(frame_id=frame_id)
        # quality reconstruction
        reconstructed = self.quality_reconstruction(matched_pairs=matched_pairs, det_bboxes=det_bboxes)

        new_prompts = np.empty((0, 4))
        obj_ids = []
        if added:
            new_prompts = np.append(new_prompts, np.array([self.tracklets[obj_id].bbox for obj_id in added], dtype=np.float32), axis=0)
            obj_ids.extend(added)
        if reconstructed:
            new_prompts = np.append(new_prompts, np.array([self.tracklets[obj_id].bbox for obj_id in reconstructed], dtype=np.float32), axis=0)
            obj_ids.extend(reconstructed)
        return new_prompts, obj_ids, removed

    def get_trackinfo(self, frame_idx, inference_state, upscale=True, device='cuda'): #TODO
        '''
        ...
        self.H_orig, self.W_orig = inference_state["video_height"], inference_state["video_width"]

        # 初始化全局掩码
        tracked_mask = torch.zeros((self.H_orig, self.W_orig), dtype=torch.bool, device=device)
        ...
        for obj_id in inference_state["obj_ids"]:
            ...
            if upscale: # 上采样到原始视频尺寸
                mask = F.interpolate(mask, size=(self.H_orig, self.W_orig), mode="bilinear", align_corners=False)[0, 0]
    ...
    def update(self, tck_ids, tck_logits, tck_bboxes, frame_id):
        ...
        for id, logit, bbox in zip(tck_ids, tck_logits, tck_bboxes):
            if id not in self.tracklets:
                self.tracklets[id] = Tracklet(track_id=id, logit=logit, bbox=bbox, frame_id=frame_id)
    ...
    def get_bboxes(self, det_bboxes, det_logits):
        ...
        det_satisfied_ids = [i for i in range(len(det_logits)) if det_logits[i] > self.confidence_th] # confidence_th=0.5,保留大于0.5的det_bboxes
        ...
        active_tracklets = [t for t in self.tracklets.values() if t.status!='lost'] # 活跃的tracking对象
        tck_bboxes = [t.bbox for t in active_tracklets]
        tck_ids = [t.id for t in active_tracklets]

        matches, unmatched_dets = self.HugarianMatch(det_bboxes=det_bboxes, tck_bboxes=tck_bboxes)
    ...
    def object_addition(self, unmatched_dets, det_bboxes, det_logtis, frame_id, tracked_mask, device='cuda'):
        '''
        ...
        added = [] # 需要输入SAM2的bbox prompts
         # 当前帧所有跟踪框构成的掩码
        if unmatched_dets:
            for det_idx in unmatched_dets: # 对每一个边界框，只有其中超过0.5的部分在untracking中才可以作为prompt
                x1, y1, x2, y2 = det_bboxes[det_idx]
                # 构建该框的掩码
                box_mask = torch.zeros((self.H_orig, self.W_orig), dtype=torch.bool, device=device)
                box_mask[y1:y2, x1:x2] = True
                if box_mask.sum()==0: # det_bbox很小
                    continue
                r = (box_mask & (~tracked_mask)).sum() / box_mask.sum() # r表示边界框内尚未跟踪的部分占边界框mask的比例
                ...
                if r>self.r: # 边界框超过0.5的部分未跟踪，作为新prompt预备输入
                    ...
                    unmatched_bbox_tracklet = Tracklet(track_id=unmatched_bbox_id, logit=det_logtis[det_idx], bbox=unmatched_bbox, frame_id=frame_id)
    ...
    def object_removal(self, frame_id):
        ...
        for id, tracklet in self.tracklets.items():
            if tracklet.status =='lost' and (frame_id-tracklet.last_seen)>self.max_missing:
                removed.append(id)
        return removed
    ...
    def HugarianMatch(self, det_bboxes, tck_bboxes):
        ...
        if len(tck_bboxes) > 0:
            ...
            row_ind, col_ind = linear_sum_assignment(cost_matrix) # row_ind 和 col_ind 是成本矩阵的最优分配矩阵索引
