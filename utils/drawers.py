...
def show_mask(image, mask, obj_id=None, alpha=0.6):
    ...
    image[mask_bool] = cv2.addWeighted(image, 1 - alpha, colored_mask, alpha, 0)[mask_bool]
    return image
...
def show_box(image, box, obj_id):
    ...
    cv2.rectangle(image, (x0, y0 - text_h - baseline), (x0 + text_w, y0), (255, 255, 255), -1)
    cv2.putText(image, label, (x0, y0 - baseline), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, thickness=1)
    return image